...
const uint16_t uwu[] = {-1, -1, 83, 52, -1, 90, 70, 81, 88, 88, -1,
-1, -1, -1, -1, -1, -1, 47, -1, -1, -1, 91, 93, 82, 92, 65, 66, 87, 28, 34,
64, 89, 94, 84, 85, 86, 67, 68, 69, 49, 50, 51, 95, 96, 33, 32, 30, 31, 25,
16, 17, 18, 19, 20, 21, 22, 23, 24, 63, 78, 26, 79, 80, -1, -1, 53, 75, 73,
55, 37, 56, 57, 58, 42, 59, 60, 61, 77, 76, 43, 44, 35, 38, 54, 39, 41, 74,
36, 72, 40, 71, 62, 15, 45, 46, 27, -1, 1, 2, 3, 4, 13, 5, 6, 7, 8};
 
char scan, pressed, sendy;
...
if( keyboard.available( ) )
  ...
  if( c > 0 )
    ...
    Serial.println( c & 0xFF, HEX );
       ...
       scan = uint16_t(c & 0x7F);
       pressed = uint16_t(c >> 15);
       sendy = 0;
      
      if (scan < 106) {
        sendy = uwu[scan];
        if (sendy == -1) return;
        if (pressed) sendy = sendy + 0x80;
        Serial.println( sendy , HEX );
        
        if (sendy == 0x34 || c == 0x8103 ) {
          delay(50);
          // Serial1.write(sendy);
          //delay(10);
          //Serial1.write(sendy+0x80);
        } else {
          Serial1.write(sendy);
        }
